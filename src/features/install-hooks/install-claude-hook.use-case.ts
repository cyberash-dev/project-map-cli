...
export class InstallClaudeHookUseCase {
	...
	async execute(args: InstallClaudeHookArgs): Promise<InstallClaudeHookResult> {
			...
			if (raw.trim().length > 0) {
				try {
					...
					this.deps.logger.error(
						`${targetPath} is not valid JSON — fix it manually or delete the file. (${err instanceof Error ? err.message : String(err)})`,
					);
		...
		if (existingIdx >= 0) {
			submitGroups.splice(existingIdx, 1);
		}
		submitGroups.push({
		...
		this.deps.logger.info(
			`installed project-map UserPromptSubmit hook → ${targetPath}`,
		);
		this.deps.logger.info(
	...
	for (let i = 0; i < groups.length; i++) {
		...
		for (const h of group.hooks) {
			if (
				...
				h.command.includes(HOOK_SIGNATURE)
			) {
				return i;
			}
		}
