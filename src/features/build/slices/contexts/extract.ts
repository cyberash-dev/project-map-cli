...
export class ContextsExtractor implements IExtractor<BoundedContext[]> {
	...
	extract(ctx: ExtractionContext): Promise<BoundedContext[]> {
		...
		const customMap = new Map(
			cfg.custom.map((c) => [normalizePath(c.path), c.role]),
		);

		const results: BoundedContext[] = [];
		for (const [dirPath, fileCount] of fileCountsByTopDir.entries()) {
			if (fileCount < cfg.minFiles && !customMap.has(dirPath)) {
				continue;
			}
			const role =
				customMap.get(dirPath) ??
				roleFromKnown(dirPath, cfg.knownRoles) ??
				"Unspecified";
...
function roleFromKnown(
	...
	for (let i = parts.length - 1; i >= 0; i--) {
		...
		if (candidate && knownRoles[candidate]) {
			return knownRoles[candidate] ?? null;
		}
	}
	return null;
}
