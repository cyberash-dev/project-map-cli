...
export function extractPythonInteractions(
	...
	for (const file of ctx.files) {
		...
		if (!file.relPath.startsWith(`${norm}/`)) {
			continue;
		}
	...
	for (const [directory, files] of byDir.entries()) {
		...
		for (const file of files) {
			...
			for (const cls of findAll(root, (n) => n.type === "class_definition")) {
				...
				if (!name || !name.endsWith("Client")) {
					continue;
				}
				clientClass = name;
				for (const method of collectMethods(cls)) {
					if (!method.startsWith("_") && !methods.includes(method)) {
						methods.push(method);
					}
				}
				const envRef = findBaseUrlSetting(cls);
				if (envRef && !baseUrlFrom) {
					baseUrlFrom = envRef;
				}
				break;
			}
