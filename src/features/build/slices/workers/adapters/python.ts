...
export function extractPythonWorkers(
	...
	for (const file of ctx.files) {
		...
		for (const cls of findAll(root, (n) => n.type === "class_definition")) {
			...
			if (!classNamePatterns.some((r) => r.test(name))) {
				continue;
			}
			...
			for (const d of dec.namedChildren.filter((c) => c.type === "decorator")) {
				...
				const text =
					callOrAttr.type === "call"
						...
						: callOrAttr.text;
				if (
					!decoratorPatterns.some((p) => text === p || text.endsWith(`.${p}`))
				) {
					continue;
				}
				const fn = dec.namedChildren.find(
...
function readSubscriptions(cls: SyntaxNode): string[] {
	...
	for (const node of findAll(cls, (n) => n.type === "identifier")) {
		if (/^EVENT_TYPE_[A-Z_][A-Z0-9_]*$/.test(node.text)) {
