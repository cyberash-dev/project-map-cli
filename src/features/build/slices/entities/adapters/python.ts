	...
	extract(ctx: ExtractionContext): Promise<Entity[]> {
		...
		for (const file of ctx.files) {
			...
			for (const cls of findClassDefs(file)) {
				...
				if (bases.some((b) => enumBases.has(b))) {
					continue;
				}
...
function readMethods(cls: SyntaxNode, includePrivate: boolean): string[] {
	...
	for (const child of body.namedChildren) {
		if (child.type === "function_definition") {
			...
			if (!includePrivate && name.startsWith("_") && !isDunder(name)) {
				continue;
			}
			names.push(name);
...
function readFields(cls: SyntaxNode): Field[] {
	...
	for (const child of body.namedChildren) {
		...
		if (child.type === "annotated_assignment") {
			const target =
				child.childForFieldName("target") ?? child.namedChildren[0];
			const annotation =
				child.childForFieldName("annotation") ?? child.namedChildren[1] ?? null;
			if (!target || target.type !== "identifier") {
				continue;
			}
			if (target.text.startsWith("_")) {
				continue;
			}
			fields.push({
				name: target.text,
				type: annotation ? annotation.text : null,
			});
		}
	}
