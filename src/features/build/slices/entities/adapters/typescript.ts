	...
	extract(ctx: ExtractionContext): Promise<Entity[]> {
		...
		for (const file of ctx.files) {
			...
			for (const cls of classesIn(file)) {
				...
				if (methods.length < 2 && bases.length === 0) {
					continue;
				}
...
function prototypeEntitiesIn(file: ParsedFile): Entity[] {
	...
	for (const assign of findAll(
		root,
		(n) => n.type === "assignment_expression",
	)) {
		collectPrototypeAssignment(assign, touch);
	}
	...
	for (const [name, group] of groups) {
		...
		if (group.methods.size < 2 && group.bases.size === 0) {
			continue;
		}
...
function collectPrototypeAssignment(
	assign: SyntaxNode,
	touch: (ctor: string) => ProtoGroup,
): void {
	...
	if (
		...
		if (outerProp.text.startsWith("_")) {
			return;
		}
	...
	if (outerObj.type === "identifier" && outerProp.text === "prototype") {
		...
		if (rhs.type === "object") {
			for (const pair of rhs.namedChildren) {
				...
				if (key && !key.text.startsWith("_")) {
					g.methods.add(key.text);
				}
			}
		}
	}
...
function typeEntitiesIn(file: ParsedFile): Entity[] {
	...
	for (const iface of findAll(
		root,
		(n) => n.type === "interface_declaration",
	)) {
		...
		const body =
			firstChild(iface, "object_type") ?? firstChild(iface, "interface_body");
		...
		if (methods.length < 2 && bases.length === 0 && fields.length < 2) {
			continue;
		}
	...
	for (const alias of findAll(
		root,
		(n) => n.type === "type_alias_declaration",
	)) {
		...
		if (methods.length < 2 && fields.length < 2) {
			continue;
		}
...
function interfaceHeritageOf(iface: SyntaxNode): string[] {
	...
	for (const c of extendsClause.namedChildren) {
		if (c.type === "type_identifier" || c.type === "identifier") {
			...
			if (
				typeName &&
				(typeName.type === "type_identifier" || typeName.type === "identifier")
			) {
