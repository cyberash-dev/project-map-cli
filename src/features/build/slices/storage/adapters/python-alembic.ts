...
export function extractPythonAlembicMigrations(
	...
	for (const file of ctx.files) {
		...
		if (!file.relPath.startsWith(`${dirNorm}/`) && file.relPath !== dirNorm) {
			continue;
		}
		const migration = migrationFromFile(file.relPath, file.tree);
		if (migration) {
...
function migrationFromFile(relPath: string, tree: unknown): Migration | null {
	...
	for (const fnDef of findAll(root, (n) => n.type === "function_definition")) {
		...
		for (const call of findAll(fnDef, (n) => n.type === "call")) {
			...
			const opMatch =
				/^op\.(create_table|drop_table|add_column|drop_column|alter_column|rename_table|create_index|drop_index)$/.exec(
					fnText,
				);
			if (!opMatch) {
				continue;
	...
	const summary =
		opsSummary.length === 0
			...
			: dedupe(opsSummary).join(", ");
	const source: SourceLocation = { file: relPath, line: 1 };
	return {
		revision,
		downRevision,
		tables: [...tables],
...
export function _internalProbe(): void {}
