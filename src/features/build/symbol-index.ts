	...
	switch (language) {
		case "python":
			for (const n of findAll(
				root,
				(x) =>
					x.type === "class_definition" || x.type === "function_definition",
			)) {
		...
		case "javascript":
			for (const n of findAll(
				root,
				(x) =>
					x.type === "class_declaration" ||
					x.type === "function_declaration" ||
					x.type === "interface_declaration" ||
					x.type === "enum_declaration",
			)) {
				push(n.childForFieldName("name"));
			}
			break;
		case "go":
			for (const n of findAll(
				root,
				(x) =>
					x.type === "type_declaration" || x.type === "function_declaration",
			)) {
				if (n.type === "function_declaration") {
					...
					for (const spec of n.namedChildren) {
						if (spec.type === "type_spec") {
							push(spec.childForFieldName("name"));
						}
					}
				}
			}
		...
		case "kotlin":
			for (const n of findAll(
				root,
				(x) =>
					x.type === "class_declaration" || x.type === "function_declaration",
			)) {
...
function factoryOf(init: SyntaxNode): string | null {
	...
	if (init.type === "new_expression") {
		...
		if (
			ctor &&
			(ctor.type === "identifier" || ctor.type === "member_expression")
		) {
...
function collectImports(root: SyntaxNode, language: Language): string[] {
	...
	switch (language) {
		case "python":
			for (const imp of findAll(
				root,
				(x) =>
					x.type === "import_from_statement" || x.type === "import_statement",
			)) {
				for (const c of imp.namedChildren) {
					...
					if (c.type === "import_list") {
						for (const el of c.namedChildren) {
							if (el.type === "dotted_name" || el.type === "identifier") {
								out.add(el.text);
							}
						}
					}
		...
		case "javascript":
			for (const imp of findAll(root, (x) => x.type === "import_statement")) {
				for (const c of findAll(
					imp,
					(x) => x.type === "import_specifier" || x.type === "identifier",
				)) {
