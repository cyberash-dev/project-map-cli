...
/**
 * Check mode writes no path. Both the document and the artifact are compared
 * byte for byte: since project-map:DLT-020 neither carries a timestamp, so
 * there is nothing left to normalize away first.
 */
...
function applyOnly<T extends { sections: readonly SectionId[] }>(
	...
	const unknown = requested.filter(
		(id) => !SECTION_IDS.some((known) => known === id),
	);
	if (unknown.length > 0) {
		throw new ConfigTimeError(
			"schema_violation",
			`--only names no accepted section id: ${unknown.join(", ")}`,
		);
	}
