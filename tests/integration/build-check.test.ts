...
describe("check mode drift classes", () => {
	...
	it("check mode ignores differences confined to non-reproducible fields", async () => {
		await runCli(workspace.dir, ["build"]);
		const mdPath = path.join(workspace.dir, MD);
		const drifted = (await readFile(mdPath, "utf8"))
			.replace(
				/^Generated by project-map v.+$/m,
				"Generated by project-map v9.9.9 on 1999-01-01T00:00:00.000Z",
			)
			.replace(/^\| Build duration\s*\|.*\|$/m, "| Build duration | 1 ms |");
