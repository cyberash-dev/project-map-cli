...
Прогресс: правки требований до v4.1 внесены; нормативные записи под весь MVP §15
заведены в `spec/detection.md` (песочница SDD, все `proposed`). Фаза A не начата
(продакшн-кода нет).

Репозиторий живёт под SDD, поэтому этот файл описывает _раскладку по коду_, а
источником истины о том, что именно гарантировано наружу, служат нормативные ID.
Соответствие фаз и ID — в шапке `spec/detection.md`; предикаты — в самих записях.
Код фазы пишется только после того, как её ID промоутнуты в `spec/spec.md` и
проведены через `sdd approve` + `sdd finalize`.
...
(v4.1) заменяет это на структурную детекцию с провенансом импортов, честным
...
`scripts/emit-build-digest.mjs` в `npm run build` · раздел
`SECTION_IDS` / `DEFAULT_SECTION_IDS` · `BuildProjectMapUseCase.execute(cwd, sources?)`.

Закрывает: `CTR-004`, `CTR-006`, `CTR-007`, `CTR-008`, `INV-003`, `POL-003`,
`DLT-004`, `IMP-006`, `IMP-007`.
...
1. Правки требований до v4.1 в `intraservice-map` (три аддитивные правки +
   фиксация отложенного) — **до кода**, чтобы реализация не расходилась с
   источником правды. Сделано.
2. Нормативные записи под весь MVP §15 в `spec/detection.md`, все `proposed`;
   `sdd lint` и `sdd ready` по нулю. Сделано.
3. Фаза A → B → C → D → E1 → E2 → F. Каждая фаза внутри себя:
   1. перенести ID фазы из `spec/detection.md` в `spec/spec.md`, применив
      правки approved-записей, которые авторизует Delta этой фазы;
   2. написать Red-тесты с `@covers <id>` — по одному за раз, падающие на
      утверждении, а не на отсутствующем импорте;
   3. `sdd approve --id <…> --approver <человек>` на все ID фазы одним планом,
      затем один `sdd finalize` (порознь упрётся в `proposed-references`);
   4. Green, затем рефакторинг под зелёными тестами;
   5. `pnpm lint`, `pnpm format:check`, `pnpm test`, `sdd lint`, `sdd ready` —
      все по нулю; рефреш `BL-001`; демонстрация на живом сервисе.
4. Коммиты — по фазам, гранулярно внутри (ядро/адаптер/фикстуры отдельно), без
   AI-атрибуции.
