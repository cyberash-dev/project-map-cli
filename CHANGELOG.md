...
Everything removed stays in `project-map.json` (`output.json`), which carries
`metadata`, `source.line`, `fileCount` and `referencedFrom` unchanged.
...
- **`init` writes the section list without `metadata`.**

### Migration

Drop `- metadata` from `sections` in `.project-map.yaml`, then rebuild and
commit `PROJECT_MAP.md`. A consumer that parsed the tool version, the
